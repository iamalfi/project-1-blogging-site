...
const createrAuthor = async function (req, res) {
    try {
        ...
        if (Object.keys(data).length == 0) {
            return res
                .status(400)
                .send({ status: false, msg: "Please enter in Body" });
        }
        if (
            !data.title == "Mr" &&
            !data.title == "Mrs" &&
            ...
            res.status(400).send({
                status: true,
                msg: "title must be Mr,Mrs,Miss",
            });
        }
        if (!data.fname) {
            return res
                .status(400)
                .send({ status: false, msg: "fname  is missing" });
        }
        ...
        if (!data.lname) {
            return res
                .status(400)
                .send({ status: false, msg: "lname is missing" });
        }
        ...
        if (!data.title) {
            return res
                .status(400)
                .send({ status: false, msg: "title is missing" });
        }
        if (data.title != "Mr" && data.title != "Mrs" && data.title != "Miss") {
            return res
                ...
                .send({
                    status: false,
                    msg: "title must be 'Mr' OR 'Mrs' OR 'Miss'",
                });
        }
        if (!data.email) {
            return res
                .status(400)
                .send({ status: false, msg: "email is missing" });
        }
        if (!data.password) {
            return res
                .status(400)
...
const login = async function (req, res) {
    try {
        ...
        if (!email || !password) {
            return res
                .status(400)
                .send({ status: false, msg: "Email or password is required" });
        }
